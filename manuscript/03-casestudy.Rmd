...
Now we perform the procedure using the 1/2(LOD) substitution censoring method, akin to @Kelderman2019's implementation:
...
avg_control <- df_std %>%
  group_by(well.id, contaminant, measurement.unit, gradient, disposal.area) %>%
  summarize(avg_conc = mean(concentration)) %>%
  ungroup()

#get highest avg. upgradient concentrations for each contaminant
highestupgrad_control <- avg_control %>%
  filter(gradient %in% "Upgradient") %>%
  group_by(contaminant, disposal.area) %>%
  summarize(max_conc = max(avg_conc)) %>%
  ungroup()

#remove any wells with average downgradient concentrations lower than the highest average upgradient concentration for that specific contaminant and disposal area
downgrad_control <- avg_control %>%
  filter(gradient %in% "Downgradient")

downgrad_control2 <- downgrad_control %>%
  select(c(well.id, contaminant, disposal.area, avg_conc))

downgrad_control3 <- left_join(downgrad_control2, highestupgrad_control) %>%
  mutate(max_conc = case_when(
    is.na(max_conc) ~ -9999999, #accounting for na's
    TRUE ~ max_conc))

downgrad_control4 <- downgrad_control3 %>% #all downgradient mean concentrations that were greater than “background” levels
  filter(avg_conc > max_conc)

#for each disposal area, identify the well(s) with the highest mean concentration of each contaminant
highest_control <- downgrad_control4 %>%
  group_by(disposal.area, contaminant) %>%
...
highest_control2 <- left_join(highest_control, hbt_case, by = "contaminant")


highest_control3 <- highest_control2 %>%
  mutate(ratio = avg_conc/as.numeric(limit)) #calculate the ratios -> ‘highest average’ concentrations/health-based threshold




#NOTE TO SELF: something to discuss in following section/exceptions: 
#while trying to match wells with their associated disposal areas, recognized that the naming conventions for disposal areas is a bit strange... for example the disposal named 'north ash basin system' has only downgradient wells but 'North Ash Basin System, RWS Type I South Landfill FP# 26-06' has only upgradient wells. as such, when we are trying to find the highest average upgradient concentration of a contaminant for a certain disposal area, we may not 100% account for all wells, since 1) we do not know if these disposal areas are even connected 2) time-constraint of going through and picking out keywords for disposal areas and matching them, etc. as such, we need to keep in mind that some exceptions may be missed...
```
