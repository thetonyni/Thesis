...
avg_mle <- df_std %>%
  ...
  group_by(well.id, contaminant, measurement.unit, gradient, site) %>%
  do(data.frame(avg_conc = get_mle_mean(avg_mle$concentration, avg_mle$censored))) %>%
  ungroup()



mle = cenmle(avg_mle$concentration, avg_mle$censored)
...
Notes: control+substitution are same top 10, but composite scores slightly differ
