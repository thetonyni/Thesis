...
As stated previously, @Kelderman2019 compiled a list of the top 10 most contaminated sites across the U.S., following the EPA's health-based threshold for specific contaminants. We will briefly follow with a discussion on their methodology in determining these top 10 most contaminated sites.
...
It is also important to note that since upgradient wells measure the quality of the water _before_ it passes through a coal site, they are also removed.
...
```{r keldermantop10, echo = FALSE, message = FALSE, cache = TRUE}
keldermantop10 <- c("San Miguel Plant",
           "Allen Steam Station",
...
keldermantop10df <- full %>%
  filter(site %in% keldermantop10)

#number of wells in kelderman's top 10 sites
keldermantop10df %>%
  group_by(site)%>%
  summarize(n = n()) %>%
  arrange(desc(n)) %>%
  kable() #temporary table (ugly)
```

```{r}
#how many well are below detection in each site
keldermantop10df %>%
