...
Using these values we can calculate what is known as the _exceedance probability_ ($pe_{i}$) for each of our $k$ LOD values, defined recursively as:
...
1. Compute normal quantiles (z-scores) by computing $z_{ij}^{u} = \Phi^{-1}(p^u_{ij})$, where $\Phi^{-1}$ is the inverse standard normal CDF.

2. Construct censored probability plots using the z-scores from step 6.

3. Plot $z_{ij}^{u}$ against a transformation, $f(\cdot)$, (ex: log, square root, inverse, etc.) of the uncensored values $x_{ij}^{u}$.

4. Compute correlation coefficient between the $z_{ij}^{u}$ and $f(x_{ij}^{u})$ pairs. The transformation which yields the highest correlation coefficient is the one which most optimizes the left-censored values.

5. Compute a linear regression of the transformed values, $f(x_{ij}^{u})$ against the z-scores, $z_{ij}^{u}$.

6. Use Eq. 1.4 to compute $p^c_{ij}$ and repeat steps 1 through 3, to obtain z-scores for the censored values.
