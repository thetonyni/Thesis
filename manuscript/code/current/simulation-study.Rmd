...
km_func <- function(n, samplesize){
   for(i in 1:n){
    ...
    store_df[i, 2] <- "km"
    store_df[i, 3] <- NADA::mean(km_res)[[1]]
    store_df[i, 4] <- NADA::sd(km_res)
  }
  return(store_df)
}
...
```{r}
testdf1 <- mosaic::do(1) * sub_func(5, 10)
testdf2 <- mosaic::do(1) * mle_func(5, 10)
testdf3 <- mosaic::do(1) * km_func(5, 10)
