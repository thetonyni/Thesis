...
## bootstrapped differences for first disposal area
first <-gather((mean_df[1,] %>% 
                  slice(rep(1:n(), each = n)))-sample_n_contam,'contaminant','concentration')
## bootstrapped differences for second disposal area
second <- gather((mean_df[2,] %>% 
                    slice(rep(1:n(), each = n)))-sample_n_contam,'contaminant','concentration')

## get median and 95% CI for each contaminant for first disposal area
first <- first %>%
  group_by(contaminant) %>%
  dplyr::summarize(med_diff = median(concentration), 
            upper = quantile(concentration,.975), 
            lower = quantile(concentration,.025))

## get median and 95% CI for each contaminant for second disposal area
second <- second %>%
  group_by(contaminant) %>%
  dplyr::summarize(med_diff = median(concentration), 
            upper = quantile(concentration,.975), 
...
## plot (these are hideous but just to get a sense of the results)
first %>%
  filter(contaminant %ni% c("Total Dissolved Solids", "pH"))%>%
  ggplot(aes(x = 0, y = med_diff)) +
  geom_point() +
  geom_errorbar(aes(ymin = lower, ymax = upper)) +
  geom_hline(yintercept = 0,col='red')+
  ggtitle("Bootstrapped Distribution of Contaminants caused by Disposal Area (n=500)") +
  ylab("Downgradient - Upgradient") +
  theme_bw()+facet_wrap(~contaminant,ncol=5,scales='free_y')
## no obvious contamination by first disposal area (upgradient and downgradient diffs mostly near 0)
...
second %>%
  ...
  ylab("Downgradient - Upgradient") +
  theme_bw()+facet_wrap(~contaminant,ncol=5,scales='free_y')
## clear contamination occuring due to second disposal area (upgradient and downgradient diffs mostly >>0)
