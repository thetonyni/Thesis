...
\caption{\label{tab:unnamed-chunk-16}Top 10 most contaminated sites 
...
\caption{\label{tab:unnamed-chunk-17}Top 10 most contaminated sites 
...
\caption{\label{tab:unnamed-chunk-18}Top 10 most contaminated sites 
...
Our top ten most contaminated sites when we don't use any methods to account for censoring caused us to obtain slightly different results than the top ten sites that Kelderman et al. (2019) obtained -- specifically Sebree Generating Station is in our top ten list while Ghent Generating Station is on theirs. Also the ranking/order of our top ten sites is slightly different as well. This should not be much of a concern however, we are more focused on the methods to handle left-censoring and observing if they alter the top ten sites obtained with this baseline implementation.

Next, we will utilize the 1/2(LOD) substitution method implemented by Kelderman et al. (2019).

MLE method troublesome to utilize:

firstly, had to remove all observations that were 0 due to inability of lognormal distribution specification to handle 0 values (log(0) = undefined), when 0 values were removed, algorithm obtained troublesome values for several sites (namely Inf, values in the millions, etc. due to high rate of censoring for some of the sites, due to this, we will not use the results from the mle method), since omitting observations whose concentrations are equal to zero already makes it difficult to compare the effectiveness of the MLE method to the other methods.

We can recall our discussion in the results for the MLE method resulting in overinflated estimates when the data being used is highly skewed.

(we may want to check to see if our data is highly skewed to prove this as well)

ROS unable to be run due to errors with \textgreater80\% of data for some groupings being censored. As stated previously, ROS is only able to be run if more than half the data is known/uncensored. As such, we are unable to implement ROS with our current dataset.
