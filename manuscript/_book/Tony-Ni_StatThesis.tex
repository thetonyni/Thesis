...
\caption{\label{tab:unnamed-chunk-9}Health-based thresholds set by EPA.}
\centering
\begin{tabular}[t]{lll}
\toprule
...
\begin{verbatim}
   ...
   2 ug/l             13908
\end{verbatim}
Now we perform the procedure using the 1/2(LOD) substitution censoring method, akin to Kelderman et al. (2019)'s implementation:
\begin{Shaded}
\begin{Highlighting}[]
...
\NormalTok{avg\_control }\OtherTok{\textless{}{-}}\NormalTok{ df\_std }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{group\_by}\NormalTok{(well.id, contaminant, measurement.unit, gradient, disposal.area) }\SpecialCharTok{\%\textgreater{}\%}
...
\end{Shaded}
\begin{verbatim}
   `summarise()` regrouping output by 'well.id', 'contaminant', 'measurement.unit', 'gradient' (override with `.groups` argument)
\end{verbatim}
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#get highest avg. upgradient concentrations for each contaminant}
\NormalTok{highestupgrad\_control }\OtherTok{\textless{}{-}}\NormalTok{ avg\_control }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{filter}\NormalTok{(gradient }\SpecialCharTok{\%in\%} \StringTok{"Upgradient"}\NormalTok{) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{group\_by}\NormalTok{(contaminant, disposal.area) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{summarize}\NormalTok{(}\AttributeTok{max\_conc =} \FunctionTok{max}\NormalTok{(avg\_conc)) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{ungroup}\NormalTok{()}
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
   `summarise()` regrouping output by 'contaminant' (override with `.groups` argument)
\end{verbatim}
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#remove any wells with average downgradient concentrations lower than the highest average upgradient concentration for that specific contaminant and disposal area}
\NormalTok{downgrad\_control }\OtherTok{\textless{}{-}}\NormalTok{ avg\_control }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{filter}\NormalTok{(gradient }\SpecialCharTok{\%in\%} \StringTok{"Downgradient"}\NormalTok{)}

\NormalTok{downgrad\_control2 }\OtherTok{\textless{}{-}}\NormalTok{ downgrad\_control }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{select}\NormalTok{(}\FunctionTok{c}\NormalTok{(well.id, contaminant, disposal.area, avg\_conc))}

\NormalTok{downgrad\_control3 }\OtherTok{\textless{}{-}} \FunctionTok{left\_join}\NormalTok{(downgrad\_control2, highestupgrad\_control) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{max\_conc =} \FunctionTok{case\_when}\NormalTok{(}
    \FunctionTok{is.na}\NormalTok{(max\_conc) }\SpecialCharTok{\textasciitilde{}} \SpecialCharTok{{-}}\DecValTok{9999999}\NormalTok{, }\CommentTok{\#accounting for na\textquotesingle{}s}
    \ConstantTok{TRUE} \SpecialCharTok{\textasciitilde{}}\NormalTok{ max\_conc))}
\end{Highlighting}
\end{Shaded}
\begin{verbatim}
   Joining, by = c("contaminant", "disposal.area")
\end{verbatim}
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{downgrad\_control4 }\OtherTok{\textless{}{-}}\NormalTok{ downgrad\_control3 }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\#all downgradient mean concentrations that were greater than “background” levels}
  \FunctionTok{filter}\NormalTok{(avg\_conc }\SpecialCharTok{\textgreater{}}\NormalTok{ max\_conc)}

\CommentTok{\#for each disposal area, identify the well(s) with the highest mean concentration of each contaminant}
\NormalTok{highest\_control }\OtherTok{\textless{}{-}}\NormalTok{ downgrad\_control4 }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{group\_by}\NormalTok{(disposal.area, contaminant) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{slice}\NormalTok{(}\FunctionTok{which.max}\NormalTok{(avg\_conc))}


\CommentTok{\# }
\CommentTok{\# TEST\_highest\_control2 \textless{}{-} left\_join(highest\_control, hbt, by)}
\CommentTok{\#   }
\CommentTok{\#   }
\CommentTok{\#   highest\_control \%\textgreater{}\%}
\CommentTok{\#   left\_join(\#get only 1 of the highest of each contaminant in a disposal area}
\CommentTok{\#   mutate(ratio = avg\_conc/) \#calculate the ratios {-}\textgreater{} ‘highest average’ concentrations/health{-}based threshold}
\CommentTok{\# }



\CommentTok{\#NOTE TO SELF: something to discuss in following section/exceptions: }
\CommentTok{\#while trying to match wells with their associated disposal areas, recognized that the naming conventions for disposal areas is a bit strange... for example the disposal named \textquotesingle{}north ash basin system\textquotesingle{} has only downgradient wells but \textquotesingle{}North Ash Basin System, RWS Type I South Landfill FP\# 26{-}06\textquotesingle{} has only upgradient wells. as such, when we are trying to find the highest average upgradient concentration of a contaminant for a certain disposal area, we may not 100\% account for all wells, since 1) we do not know if these disposal areas are even connected 2) time{-}constraint of going through and picking out keywords for disposal areas and matching them, etc. as such, we need to keep in mind that some exceptions may be missed...}
\end{Highlighting}
\end{Shaded}
After these values are obtained, any wells with average downgradient concentrations lower than the highest average upgradient concentration for that specific contaminant and disposal area were removed. It is also important to note that since upgradient wells measure the quality of the water \emph{before} it passes through a coal site, they are also removed.
