...
Let \(LOD_k\) be the \(k^{th}\) distinct LOD value for a variable with \(K\) distinct limits of detection. \(A_k\) as the number of uncensored values between the \(i\)th and \((i+1)\)th LOD for \(k=1\) to \(K-1\), and \(A_k\) as the number of uncensored values above the highest LOD, \(A_0\) as the number of uncensored values below the lowest LOD.
...
We can then write the number of censored values below the \(i\)th LOD (\(C_i\)) as:

\[\textrm{Eq. 1.1:} \ \  C_i = B_i - B_{i-1}-A_{i-1}\]

for \(i = 1\) to \(k\).

Using these values we can calculate what is known as the \emph{exceedance probability} (\(pe_{i}\)) for each of our \(k\) LOD values, defined recursively as:

\[\textrm{Eq. 1.2:} \ \  p^e_i = p^e_{i+1} + \frac{A_i}{A_i+B_i}(1 - p^e_{i+1})\]
