...
{\centering \includegraphics[width=1\linewidth]{Tony-Ni_StatThesis_files/figure-latex/rosexample-1} 
...
\item
  Compute normal quantiles (z-scores) by computing \(z_{kj}^{u} = \Phi^{-1}(p^u_{kj})\) for the uncensored values and \(z_{kj}^{c} = \Phi^{-1}(p^c_{kj})\) where \(\Phi^{-1}\) is the inverse standard normal CDF.
\item
  Plot the uncensored values against their z-scores, this is what is known as the censored probability plot.
\item
  If a linear trend is not visible, we may want to transform the uncensored values using some function \(f(\cdot)\) to better satisfy linearity.
\item
  Regress \(f(x_{kj}^{u})\) against the z-scores, \(z_{kj}^{u}\).
\item
  Obtain the transformed imputed values using the slope and intercept from the regression model from step 4 and the censored z-scores from step 1 using the following expression:
\end{enumerate}
...
\tightlist
\item
  Backtransform the imputed values for the censored values (if necessary) and combine them with the uncensored values to obtain a new dataset from which we can obtain estimates for the parameters of interest.
\end{enumerate}
As we have now finished discussing how the ROS method works, we must turn to the limitations of the ROS method. While ROS has been seen as a popular method for cases when it is unclear if the data can be properly modeled with a parametric distribution, it has several limitations as well (Lafleur et al., 2011). In order for ROS to be utilized, there needs to be at least 5 known values and more than half the values within the censored variables must be known. As regression is utilized in this method, the response variable must also be a linear function of the explanatory variable (quantiles). Additionally, the errors should have constant variance (Lee \& Helsel, 2005).
...
{\centering \includegraphics[width=1\linewidth]{Tony-Ni_StatThesis_files/figure-latex/lnfigure-1} 
...
{\centering \includegraphics[width=1\linewidth]{Tony-Ni_StatThesis_files/figure-latex/expfigure-1} 
...
{\centering \includegraphics[width=1\linewidth]{Tony-Ni_StatThesis_files/figure-latex/wfigure-1} 
...
\addlinespace
Site & The name of the site, as presented in the
             groundwater monitoring report.\\
\addlinespace
Disposal Area & The name of the disposal area(s), as presented 
             in the groundwater monitoring report.\\
\addlinespace
Type of Well & The type of disposal unit: surface impoundment, 
             landfill, and mixed multi-unit.\\
\addlinespace
ID of Well & The identifier given to each monitoring well in the 
             groundwater monitoring report.\\
\addlinespace
Gradient Type & The location of the groundwater monitoring well: 
             upgradient or downgradient.\\
\addlinespace
Sample Date & The date the well was sampled.\\
\addlinespace
Contaminant Name & The contaminant name.\\
\addlinespace
Measurement Unit & The concentration units. These include mg/l, ug/l, 
             and pCi/l.\\
\addlinespace
Below Detection & LOD status: '<' indicates
             the concentration is below the LOD.\\
\addlinespace
Concentration & The concentration of the contaminant.\\
\bottomrule
\end{tabular}}
\end{table}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\#function will generate a vector of numbers from the log{-}normal }
...
\NormalTok{            ) }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{mutate}\NormalTok{(}\AttributeTok{prop\_cens =} \FunctionTok{as.factor}\NormalTok{(prop\_cens),}
         \AttributeTok{samplesize =} \FunctionTok{as.factor}\NormalTok{(samplesize))}
\end{Highlighting}
\end{Shaded}
\hypertarget{weibull}{%
...
\NormalTok{avg\_km }\OtherTok{\textless{}{-}}\NormalTok{ df\_std }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{group\_by}\NormalTok{(well.id, contaminant, measurement.unit, gradient, site) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{do}\NormalTok{(}\FunctionTok{summarize}\NormalTok{(., }\AttributeTok{avg\_conc =} 
                 \FunctionTok{get\_km\_mean}\NormalTok{(.}\SpecialCharTok{$}\NormalTok{concentration, .}\SpecialCharTok{$}\NormalTok{censored))) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{ungroup}\NormalTok{()}
