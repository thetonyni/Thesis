...
In summary, ROS imputes the censored data using the estimated parameters from the linear regression model of the uncensored observed values versus their quantiles.

There are of course, some requirements which must hold in order for ROS to be utilized, at minimum, there needs to be at least 3 known values and more than half the values within the data set must be known. As regression is utilized in this method, {[}STOPPED HERE, CONTINUE HERE{]}

Its potential comes out when around 50 to 80\% of the data is below the limit of detection or when estimates of the mean and/or standard deviation are necessary (Lee \& Helsel, 2005).
