...
There is no universal reporting practice for values below the LOD which can lead to confusion amongst researchers. LOD values are often overlooked which can bring about faulty analysis and/or conclusions which are heavily flawed.

One of the most common approaches used to account for left-censored data is omitting censored values from the analysis, due to its ease of implementation, but this approach discards a myriad of useful information. 
...
Another issue lies within the art of reporting values which fall below the LOD. Amongst chemists, indicating whether or not if a value is below the LOD varies widely across labs and as reporting practices are not standardized. Some laboratories may explicitly record `ND`, others may put down `<` followed by the smallest recorded value to indicate that an observation's value is left-censored, others may simply omit the value completely [@Berthouex2020].
